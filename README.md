...
- **Полный редизайн UI.** Новый визуальный язык: pro-audio инструмент,
  плотная сетка, янтарный сигнальный акцент (как у LED-VU-метра),
  типопара Geist + Geist Mono с tabular-nums во всех числовых
  readout-ах. Light-тема перешла на тёплую фарфоровую базу (не
  инверсия dark-а), общий акцент — связь между темами.
- **Шапка переразмечена в три зоны** — бренд, master-блок (главный
  объект шапки с большим моно-readout-ом), глобальные контролы. Тогл
  «показывать все устройства» переехал в шапку панели устройств как
  сегментированный chip-переключатель.
- **Список устройств заметно перерисован.** Активные устройства
  отличаются янтарной боковой планкой и tinted-фоном — видно сразу,
  без чтения action-кнопки. Default-устройство — отдельный pill-tag
  в зелёном. Source — inline warn-блок внутри карточки (вместо
  размытого тонирования всего блока).
- **Микшер собран по-новому.** Header строки: имя + group-pill +
  быстрые actions (mute / rename / reset). Primary: Volume с
  встроенным L/R-peak-метром и dB-шкалой + Latency с tick-марками.
  Balance + EQ ушли в collapsible «Advanced» — открывается, если
  у устройства есть нестандартные значения.
- **Sync переработан в единый widget**: лейбл + sparkline истории
  дрейфа (16 Гц, цвет переключается ok↔drift) + numeric drift/target +
  кнопка-toggle «Linked». Заменил два мелких бейджа.
- **Empty states**: «Движок остановлен» — концентрические dashed-ring
  - EQ-глиф + клавиатурный хинт `Space`. «Нет активных» — стрелка
  влево к списку устройств. Стало понятно, что делать дальше.
- **Update переехал в плавающий toast** в правый верхний угол —
  больше не двигает контент при появлении / прогрессе. Footer —
  timeline последних трёх статусов с opacity-fade старых.
- **Иконки вместо эмодзи.** Единый inline-SVG sprite (`icons.js`,
  ~26 символов) — всё через `currentColor`, размер `.icon`/`.icon-lg`.
  Кросс-платформенно стабильнее, чем 🔇🔊🌓⏹⟲.
- **Окно стартует 1280 × 800 и по центру экрана** — на узких ноутбук-
  дисплеях больше не нужно ловить углы. Лого в шапке — `logo.png`
  из репо вместо placeholder-марки.
- **i18n: 19 новых ключей** (nav-mixer, filter-*, tag-*, sync-drift,
  sync-target, ctrl-advanced, empty-engine-*, empty-active-*,
  warning-source-*, update-available-title, group-add, group-members).
  Парность ru/en проверяется build.rs-ом при сборке.

## Что нового в 0.4.3

- **Семь тем оформления вместо двух.** К `dark` и `light` добавлены
  `midnight` (полночь, глубокий фиолетово-синий), `sunset` (тёплый
  персиково-коралловый, светлая), `forest` (тёмная лесная), `ocean`
  (тёмная морская, бирюза + мята) и `mono` (высококонтрастная для
  скриншотов / стресс-теста палитры).
- **Theme picker вместо кнопки-цикла.** Иконка 🎨 в шапке открывает
  попап с сеткой свотчей (превью из трёх ключевых цветов на тему) —
  тема применяется по клику.
- **Auto-режим вынесен из кнопки** в отдельный чекбокс «Следить за
  системой» внутри picker-а. При его включении атрибут `data-theme`
  снимается с `<html>`, и палитра выбирается через CSS
  `@media (prefers-color-scheme)` без участия JS — реакция мгновенная,
  ручные пресеты подсвечиваются disabled.
- **Тест-сигнал кликается.** На `.device-row` стояло
  `contain: layout paint` — `paint`-containment клиппил попап тест-
  сигнала (он позиционирован абсолютно под строкой), и нажатие 🔊
  визуально не давало ничего. Оставил `contain: layout`, попап снова
  открывается.
- **Алиас устройства теперь подставляется в предупреждение** про
  default-источник и в статус «устройство добавлено». Формат —
  «<алиас> (<системное имя>)», чтобы пользователь сразу видел и своё
  название, и оригинал в скобках. После переименования баннер
